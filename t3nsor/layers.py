...
class TTLinear(nn.Module):
    def __init__(self, in_features=None, out_features=None, bias=True, init=None, shape=None,
                 auto_shapes=True, d=2, tt_rank=2, auto_shape_mode='ascending',
                 auto_shape_criterion='entropy', forward_mode="auto"
                 ):
    ...
    def forward(self, x):
        ...
        if self.bias is None:
            x= self.mm_op(x, weight)
        else:
            x= self.mm_op(x, weight) + self.bias
